...
function ExploreContent() {
  ...
  // Sincroniza aba ativa e filtros com a query string ?tab=... ao carregar/alterar URL
  useEffect(() => {
    const tabParam = searchParams.get("tab");
    if (tabParam === "produtos" || tabParam === "luthiers" || tabParam === "professores") {
    ...
    <div className="min-h-screen bg-[#0b0908] text-surface-50 font-sans relative overflow-x-hidden">
      ...
      <footer className="border-t border-[#1c1a19]/60 mt-12 sm:mt-16 py-6 sm:py-8 text-center text-xs text-surface-500 font-body">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex flex-col items-center gap-4">
          ...
          <p>&copy; {new Date().getFullYear()} Vibrattoo. Todos os direitos reservados.</p>
        </div>
      </footer>
    </div>
  );
    ...
    <Suspense fallback={
      <div className="min-h-screen bg-surface-950 flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-surface-400">Carregando exploração...</p>
        </div>
      </div>
    }>
      <ExploreContent />
